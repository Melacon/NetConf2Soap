...
public class NetconfTagList {
    ...
    private final List<NetconfTag> tags = new ArrayList<>();
    ...
    /**
     * Read part of the Tag list.
     * @param start is the first to read, beginning at 0
     * @param number is the number of elements to read
     * @return String with xPath
     */
    public String getSubTreePath(int start, int number) {
        ...
        if (start < end && end <= tags.size()) {
            ...
            log.warn("Problem reading netconf tags: start:"+start+" number:"+number+" Targs:"+tags);
        }
        return res.toString();
    ...
    /**
     * @return: All elements of the received message
     */
    public String getSubTreePath() {
        return getSubTreePath(0, tags.size());
        /*
        StringBuilder res = new StringBuilder();
        for (NetconfTag tag : tags) {
            if (tag.hasOneValue()) {
